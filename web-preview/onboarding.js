...
(function(){
  ...
  const ROLES_WELCOME=[
    ...
    {
      title:'Как начать',
      body:'<p>1. Выберите роль ниже и войдите (PIN выдаёт администратор).</p><p>2. После входа покажем <strong>короткий тур</strong> по экрану.</p><p>3. Полная текстовая инструкция — <a href="help.html" target="_blank" rel="noopener">на странице «Помощь»</a>.</p>'
    }
  ];
  ...
  const TOURS={
    admin:[
      ...
      {
        title:'Разделы',
        body:'<p><strong>Заявки</strong> — список заказов. <strong>ЕТО</strong> — техосмотры по авто. <strong>Биржа</strong> — общие заявки между фирмами.</p><p><strong>Справочники</strong> — водители, авто, компании, тарифы.</p>',
      ...
      {
        title:'Новый заказ',
        body:'<p>Кнопка <strong>+ Заказ</strong> — создать заявку: заказчик, маршрут, цены, назначить водителя или отправить на биржу.</p>',
        target:'admin-new',
        placement:'bottom',
        onEnter(){
      ...
      },
      {
        ...
        body:'<p>Отфильтруйте заявки: все, назначенные, в работе, закрытые. Ниже — фильтр по фирме (если несколько парков).</p>',
